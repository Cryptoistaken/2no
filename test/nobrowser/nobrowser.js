...
async function cfRequestAuth(token, body) {
  const resp = await cfClient.request(API, body, { headers: { 'x-auth-token': token } })
...
async function buyNumbers(token, count, solver, captchaPromises, chatId, onProgress) {
  const bought = []
  for (let i = 0; i < count; i++) {
    log.info(`buying number ${i + 1}/${count}`, chatId)
    if (onProgress) onProgress('progress', `Buying number ${i + 1}/${count}`)

    const avail = await cfRequestAuth(token, { id: 310 })
